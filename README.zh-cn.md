...
| <img width="48px" src="https://avatars.githubusercontent.com/u/267193182?v=4" alt="Codex" /> | [Codex](https://github.com/openai/codex) | `~/.codex/sessions/` | ✅ 支持 |
...
- **交互式 TUI 模式** - 由 Ratatui 驱动的精美终端 UI（默认模式）
  ...
  - 实时筛选和排序
  - 零闪烁渲染
- **多平台支持** - 跟踪 OpenCode、Claude Code、Codex、Copilot CLI、Cursor IDE、Gemini CLI、Amp、Codebuff、Droid、OpenClaw、Hermes Agent、Pi、Kimi CLI、Qwen CLI、Roo Code、Kilo、Mux、Kilo CLI、Crush、Goose、Antigravity、Zed、Kiro、Trae、Gajae-Code、Grok Build 和 Synthetic 的使用情况
...
Tokscale 可以聚合来自 **Codex 无头输出**的令牌使用情况，用于自动化、CI/CD 流水线和批处理。

**什么是 Headless 模式？**

当您使用 JSON 输出标志运行 Codex 时（例如 \`codex exec --json\`），它会将使用数据输出到 stdout，而不是存储在常规会话目录中。Headless 模式允许您捕获和跟踪这些使用情况。
...
> **注意**：无头捕获仅支持 Codex。如果直接运行 Codex，必须如上所示将 stdout 重定向到 headless 目录。
...
| Codex | `~/.codex/` | `%USERPROFILE%\.codex\` | 可通过 `CODEX_HOME` 环境变量配置（[源码](https://github.com/openai/codex)） |
...
| Codex | 禁用 | N/A | 无清理功能 | [#6015](https://github.com/openai/codex/issues/6015) |
...
Codex 没有内置会话清理。`~/.codex/sessions/` 中的会话无限期保留。
